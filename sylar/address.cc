...
namespace sylar{
    ...
    std::shared_ptr<IPAddress> Address::LookupAnyIPAdd(const std::string& host,int family,int type,
            int protocol){
                std::vector<Address::ptr> result;
                if(Lookup(result,host,family,type,protocol)){
                    for(auto& i:result){
    ...
    bool Address::GetInterfaceAddresses(std::vector<std::pair<Address::ptr,uint32_t> >& result,const std::string& iface,
            int family){
                ...
                for(;its.first!=its.second;++its.first){
                    result.push_back(its.first->second);      //its.first是迭代器。
                }
    ...
    bool Address::Lookup(std::vector<Address::ptr>& result,const std::string& host,int family,int type,
        ...
        //检查node service
        if(node.empty()){
                service=(const char*)memchr(host.c_str(),':',host.size());   //可能是像 mail.google.com 最前面是服务。这里是也有可能服务在后面
                if(service){
    ...
    IPAddress::ptr IPAddress::Create(const char* address,uint16_t port){   //由明文得到sockaddr
        ...
        try{
            IPAddress::ptr result=std::dynamic_pointer_cast<IPAddress>(Address::Create(results->ai_addr,(socklen_t)
            results->ai_addrlen));
            if(result){
                result->setPort(port);
            }
    ...
    IPAddress::ptr IPv6Address::broadcastAddress(uint32_t prefix_len) {
        ...
        for(uint32_t i=prefix_len/8+1;i<16;i++){
            baddr.sin6_addr.s6_addr[i]=0xff;        //低位是左边的。（注意字节序）
        }
    ...
    static const size_t  MAX_PATH_LEN =sizeof(((sockaddr_un*)0)->sun_path)-1;    //sizeof包括char结尾的"\0"  char a[19]有效的只有18个。
