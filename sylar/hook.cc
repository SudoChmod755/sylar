...
namespace sylar{
    ...
    uint32_t event,int timeout_so,Args&&... args){            //右值   //ssize_t 是 signed size_t
...
retry:
        ...
        if(n==-1 && errno==EAGAIN){     //EAGAIN非阻塞读写失败（比如没有数据可读）
            ...
            if(rt){
                ...
                sylar::Fiber::YieldToHold();    //epoll_wait事件会schedule当前fiber,那时候回来。 也有可能是超时了回来（这是因为cancelEvent里的trigger）
                if(timer){
                    timer->cancel();
                }
                if(tinfo->cancelled){
...
extern "C"{
    ...
    unsigned int sleep(unsigned int seconds){
        ...
        //     iom->schedule(fiber);
        // });
        iom->addtimer(seconds*1000,std::bind((void(sylar::Scheduler::*)(sylar::Fiber::ptr,int thread))&sylar::IOManger::schedule,   //这个是模板函数,这里声明了模板类型。
        iom,fiber,-1));
...
int connect_with_timeout(int fd, const struct sockaddr* addr, socklen_t addrlen, uint64_t timeout_ms) {
    ...
    if(timeout_ms != (uint64_t)-1) {
        timer = iom->addConditionTimer(timeout_ms,winfo,[winfo, fd, iom]() {
                auto t = winfo.lock();
                if(!t || t->cancelled) {
                    return;
                }
    ...
    int rt = iom->addEvent(fd, sylar::IOManger::WRITE);
    if(rt == 0) {
        sylar::Fiber::YieldToHold();
        if(timer) {
            timer->cancel();
...
int connect(int sockfd, const struct sockaddr *addr,
    ...
    ssize_t read(int fd, void *buf, size_t count){
        return do_io(fd,read_f,"read",sylar::IOManger::READ,SO_RCVTIMEO,buf,count);
    }
    ...
    return do_io(fd, write_f, "write", sylar::IOManger::WRITE, SO_SNDTIMEO, buf, count);
}
...
ssize_t send(int s, const void *msg, size_t len, int flags) {
    return do_io(s, send_f, "send", sylar::IOManger::WRITE, SO_SNDTIMEO, msg, len, flags);
}

ssize_t sendto(int s, const void *msg, size_t len, int flags, const struct sockaddr *to, socklen_t tolen) {
    return do_io(s, sendto_f, "sendto", sylar::IOManger::WRITE, SO_SNDTIMEO, msg, len, flags, to, tolen);
}

ssize_t sendmsg(int s, const struct msghdr *msg, int flags) {
    return do_io(s, sendmsg_f, "sendmsg", sylar::IOManger::WRITE, SO_SNDTIMEO, msg, flags);
}
