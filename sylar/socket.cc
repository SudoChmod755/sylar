...
namespace sylar{
    ...
    void Socket::setSendTimeout(int64_t v){
        struct timeval tv {int(v/1000),int(v%1000*1000)};   //秒和微秒。
    ...
    void Socket::setRecvtimeout(int64_t v){
        struct timeval tv {int(v/1000),int(v%1000*1000)};   //秒和微秒。
        setOption(SOL_SOCKET,SO_RCVTIMEO,tv);         //SOL_SOCKET套接字级别。
    }
    ...
    bool Socket::connect(const Address::ptr addr,uint64_t timeout_ms){
        ...
        if(timeout_ms==(uint64_t)-1){
            ...
            if(::connect_with_timeout(m_sock,addr->getAddr(),addr->getAddrLen(),timeout_ms)){
                SYLAR_LOG_ERROR(g_logger)<<"sock="<<m_sock<<" connect("<<addr->toString()
                <<") error timeout="<<timeout_ms<<"errno="<<errno<<" errstr="<<strerror(errno);
                close();
                return false;
            }
        }
    ...
    Address::ptr Socket::getRemoteAddress(){
        ...
        if(getpeername(m_sock,result->getAddr(),&addrlen)){
            ...
            return Address::ptr(new UnknownAddress());
        }
    ...
    bool Socket::cancelRead(){
        return IOManger::GetThis()->cancelEvent(m_sock,sylar::IOManger::READ);
    }
    bool Socket::cancelWrite(){
        return IOManger::GetThis()->cancelEvent(m_sock,sylar::IOManger::WRITE);
    }
    bool Socket::cancelAccept(){
        return IOManger::GetThis()->cancelEvent(m_sock,sylar::IOManger::READ);
    }
    bool Socket::cancelAll(){
        return IOManger::GetThis()->cancelAll(m_sock);
    }
